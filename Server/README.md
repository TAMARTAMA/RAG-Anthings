...
Both the LLM and search servers are addressed via URLs in `config.json`.
...
## Quick Start
```bash
cd /home/tamar/B/code/SERVER/Server
python -m venv .venv
source .venv/bin/activate
pip install -r requirements.txt
uvicorn app.main:app --reload --host 0.0.0.0 --port 8002
```
The server also supports running with `python app/main.py` (no auto-reload).
